...
function moonraker_obico_setup_dialog() {
  status_msg "Initializing Obico for Klipper installation ..."
  ...
  if (( moonraker_count == 0 )); then
    ...
    log_error "Obico for Klipper setup started without Moonraker being installed. Aborting setup."
  ...
  if (( allowed_moonraker_obico_count > 0 )); then
    ...
    if (( moonraker_count == 1 )); then
      ...
      if (( existing_moonraker_obico_count > 0 )); then
        printf "|${green}%-55s${white}|\n" " ${existing_moonraker_obico_count} Obico for Klipper instances already installed!"
      ...
      while [[ ! ${new_moonraker_obico_count} =~ ${re} || ${new_moonraker_obico_count} -gt ${allowed_moonraker_obico_count} ]]; do
        read -p "${cyan}###### Number of new Obico for Klipper instances to set up:${white} " -i "${allowed_moonraker_obico_count}" -e new_moonraker_obico_count
        ...
        (( new_moonraker_obico_count > allowed_moonraker_obico_count )) && error_msg "Number of Obico for Klipper instances larger than installed Moonraker instances"
    ...
    local yn
    while true; do
      (( new_moonraker_obico_count == 1 )) && local question="Install Obico for Klipper?"
      (( new_moonraker_obico_count > 1 )) && local question="Install ${new_moonraker_obico_count} Obico for Klipper instances?"
      read -p "${cyan}###### ${question} (Y/n):${white} " yn
      case "${yn}" in
        Y|y|Yes|yes|"")
          select_msg "Yes"
        ...
        N|n|No|no)
          select_msg "No"
          abort_msg "Exiting Obico for Klipper setup ...\n"
          return;;
        *)
          error_msg "Invalid Input!";;
      esac
  ...
  if (( new_moonraker_obico_count > 0 )); then
    ...
    (( new_moonraker_obico_count > 1 )) && status_msg "Installing ${new_moonraker_obico_count} Obico for Klipper instances ..."
    (( new_moonraker_obico_count == 1 )) && status_msg "Installing Obico for Klipper ..."
  ...
  if (( ${#not_linked_instances[@]} > 0 )); then
    top_border
    if (( moonraker_count == 1 )); then
      printf "|${green}%-55s${white}|\n" " Obico for Klipper not linked to the server!"
    else
      printf "|${green}%-55s${white}|\n" " ${#not_linked_instances[@]} Obico for Klipper instances not linked to the server!"
...
function clone_moonraker_obico() {
  ...
  status_msg "Cloning Obico for Klipper from ${repo} ..."
  ### force remove existing Obico for Klipper dir
  [[ -d "${MOONRAKER_OBICO_DIR}" ]] && rm -rf "${MOONRAKER_OBICO_DIR}"

  cd "${HOME}" || exit 1
  if ! git clone "${repo}" "${MOONRAKER_OBICO_DIR}"; then
    print_error "Cloning Obico for Klipper from\n ${repo}\n failed!"
    exit 1
  fi
}
...
function remove_moonraker_obico_systemd() {
  ...
  status_msg "Removing Obico for Klipper Systemd Services ..."
  ...
  ok_msg "Obico for Klipper Services removed!"
}
...
function remove_moonraker_obico_dir() {
  ...
  status_msg "Removing Obico for Klipper directory ..."
...
function remove_moonraker_obico() {
  ...
  print_confirm "Obico for Klipper was successfully removed!"
  return
}
...
function update_moonraker_obico() {
  ...
  if [[ ! -d ${MOONRAKER_OBICO_DIR} ]]; then
    ...
    status_msg "Updating Obico for Klipper ..."

...
from components.katapult import KATAPULT_DIR
...
def get_sd_flash_board_list() -> List[str]:
    if not KATAPULT_DIR.exists() or not SD_FLASH_SCRIPT.exists():
        return []
...
def start_flash_process(flash_options: FlashOptions) -> None:
    ...
    try:
        ...
        process = Popen(cmd, cwd=KATAPULT_DIR, stdout=PIPE, stderr=STDOUT, text=True)
        log_process(process)
...
def run_make_clean(kconfig=Path(KATAPULT_DIR.joinpath(".config"))) -> None:
    try:
        run(
...
def run_make_menuconfig(kconfig=Path(KATAPULT_DIR.joinpath(".config"))) -> None:
    try:
        run(
...
def run_make(kconfig=Path(KATAPULT_DIR.joinpath(".config"))) -> None:
    try:
        run(

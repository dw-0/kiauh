...
class TelegramBotExtension(BaseExtension):
    def install_extension(self, **kwargs) -> None:
        Logger.print_status(_tr("Installing Moonraker Telegram Bot ..."))
        ...
        if not mr_instances:
            Logger.print_dialog(
                ...
                [
                    ...
                    _tr("Moonraker Telegram Bot requires Moonraker to be installed. Please install Moonraker first!"),
                ],
            )
            return
        ...
        Logger.print_dialog(
            ...
            [
                ...
                *instance_names,
                "\n\n",
                _tr("The setup will apply the same names to Telegram Bot!"),
            ],
        )
        if not get_confirm(
            _tr("Continue Moonraker Telegram Bot installation?"),
            default_choice=True,
            allow_go_back=True,
        ):
            return

        create_example_cfg = get_confirm(_tr("Create example telegram.conf?"))
        ...
        try:
            ...
            for name in tb_names:
                ...
                if create_example_cfg:
                    Logger.print_status(
                        _tr("Creating Telegram Bot config in {} ...").format(
                            instance.base.cfg_dir
                        )
                    )
                    ...
                    if not target_file.exists():
                        ...
                        Logger.print_info(
                            _tr("Telegram Bot config in {} already exists! Skipped ...").format(
                                instance.base.cfg_dir
                            )
                        )
            ...
            if show_config_dialog:
                Logger.print_dialog(
                    ...
                    [
                        _tr("During the installation of the Moonraker Telegram Bot, a basic config was created per instance. You need to edit the config file to set up your Telegram Bot. Please refer to the following wiki page for further information:"),
                        "https://github.com/nlef/moonraker-telegram-bot/wiki",
                    ],
                    margin_bottom=1,
                )

            Logger.print_ok(_tr("Telegram Bot installation complete!"))
        except Exception as e:
            Logger.print_error(
                _tr("Error during installation of Moonraker Telegram Bot:\n{}").format(e)
            )

    def update_extension(self, **kwargs) -> None:
        Logger.print_status(_tr("Updating Moonraker Telegram Bot ..."))
    ...
    def remove_extension(self, **kwargs) -> None:
        Logger.print_status(_tr("Removing Moonraker Telegram Bot ..."))
        ...
        try:
            ...
            Logger.print_error(_tr("Error during removal of Moonraker Telegram Bot:\n{}").format(e))

        Logger.print_ok(_tr("Moonraker Telegram Bot removed!"))
    ...
    def _remove_bot_dir(self) -> None:
        if not TG_BOT_DIR.exists():
            Logger.print_info(_tr("'{}' does not exist. Skipped ...").format(TG_BOT_DIR))
            return
    ...
    def _remove_bot_env(self) -> None:
        if not TG_BOT_ENV.exists():
            Logger.print_info(_tr("'{}' does not exist. Skipped ...").format(TG_BOT_ENV))
            return
    ...
    def _delete_bot_logs(self, instances: List[MoonrakerTelegramBot]) -> None:
        ...
        if not all_logfiles:
            Logger.print_info(_tr("No Moonraker Telegram Bot logs found. Skipped ..."))
            return
